...
const SalariesDAO = {
  salaryPaid: (payload) => {
    ...
    let totalAmount = (payload.amount.food||0 )+ (payload.amount.rent||0) + (payload.amount.living||0);
    ...
    return new SalariesModel({
      ...payload,salariesPaidId:`SALARIES-${payload.userName.toUpperCase()}-${Utility.generateUID()}-AVTSL-${getDateCode}`,totalAmount
    }).save();
  },
